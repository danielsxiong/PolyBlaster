	...
	if (World)
	{
		...
		if (ConfirmHitResult.bBlockingHit) // we hit the head, return early
		{
			/*if (ConfirmHitResult.Component.IsValid())
			{
				...
				if (Box)
				{
					DrawDebugBox(World, Box->GetComponentLocation(), Box->GetScaledBoxExtent(), Box->GetComponentQuat(), FColor::Red, false, 8.f);
				}
			}*/
		...
		else // we didn't hit the head, check the rest of the boxes
		{
			...
			if (ConfirmHitResult.bBlockingHit) // we hit something but not the head, return early
			{
				/*if (ConfirmHitResult.Component.IsValid())
				{
					...
					if (Box)
					{
						DrawDebugBox(World, Box->GetComponentLocation(), Box->GetScaledBoxExtent(), Box->GetComponentQuat(), FColor::Blue, false, 8.f);
					}
				}*/
...
FServerSideRewindResult ULagCompensationComponent::ProjectileConfirmHit(const FFramePackage& Package, APBCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	if (!HitCharacter) return FServerSideRewindResult();
	...
	else // we didn't hit the head, check the rest of the boxes
	{
		...
		if (PathResult.HitResult.bBlockingHit) // we hit something but not the head, return early
		{
			if (PathResult.HitResult.Component.IsValid())
			{
				UBoxComponent* Box = Cast<UBoxComponent>(PathResult.HitResult.Component);
...
FShotgunServerSideRewindResult ULagCompensationComponent::ShotgunConfirmHit(const TArray<FFramePackage>& Packages, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	for (auto& HitLocation : HitLocations)
	{
		...
		if (World)
		{
			...
			if (PBCharacter) // we hit something except the head
			{
				if (ConfirmHitResult.Component.IsValid())
				{
					UBoxComponent* Box = Cast<UBoxComponent>(ConfirmHitResult.Component);
...
FServerSideRewindResult ULagCompensationComponent::ProjectileServerSideRewind(APBCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	// Frame package that we check to verify a hit
	FFramePackage FrameTocheck = GetFrameToCheck(HitCharacter, HitTime);
...
void ULagCompensationComponent::ServerScoreRequest_Implementation(APBCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation, float HitTime)
{
...
void ULagCompensationComponent::ProjectileServerScoreRequest_Implementation(APBCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
...
void ULagCompensationComponent::ShotgunServerScoreRequest_Implementation(const TArray<APBCharacter*>& HitCharacters, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations, float HitTime)
{
	...
	for (auto& HitCharacter : HitCharacters)
	{
		...
		{
			float HeadshotDamage = ShotgunConfirm.HeadShots[HitCharacter] * Character->GetEquippedWeapon()->GetHeadShotDamage();
			TotalDamage += HeadshotDamage;
		}
		if (ShotgunConfirm.BodyShots.Contains(HitCharacter))
		{
			float BodyshotDamage = ShotgunConfirm.BodyShots[HitCharacter] * Character->GetEquippedWeapon()->GetDamage();
			TotalDamage += BodyshotDamage;
		}

		UGameplayStatics::ApplyDamage(HitCharacter, TotalDamage, HitCharacter->Controller, Character->GetEquippedWeapon(), UDamageType::StaticClass());
	}
}
